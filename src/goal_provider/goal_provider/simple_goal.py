...
class SimpleGoalProvider(Node):
    def __init__(self):
        ...
        f = self.tfBuffer.wait_for_transform_async(
            self.ego_frame, self.reference_frame, rclpy.time.Time().to_msg()
        )
        self.get_logger().info("waiting for transform map -> baselink")
        rclpy.spin_until_future_complete(self, f)
        self.create_timer(0.1, self.timer_cb)
        self.get_logger().info("goal_provider started")
    ...
    def increment_goal(self):
        ...
        self.get_logger().info(
            f"goal completed, going to goal {self.current_goal_index}"
        )
