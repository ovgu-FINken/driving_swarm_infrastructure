...
def path_segment(
    ...
    if model == Vehicle.DUBINS:
        ...
        # rotate (1)
        x = rtr_segment(wp1, wp2, s, model, r_step, path)
        ...
        # cleanup angles
        i, path = fix_angle_segment(FIX_ANGLES, part, path, wp2)
    elif model == Vehicle.BEZIER:
        control_point1 = (
            wp1[0] + np.sin(wp1[2]) * r,
            wp1[1] + np.cos(wp1[2]) * r,
        )
        control_point2 = (
            wp2[0] - np.sin(wp2[2]) * r,
            wp2[1] - np.cos(wp2[2]) * r,
        )
        points = []
        ...
        length = np.linspace(0.0, 1.0, num=int(curve.length / s))
        points = curve.evaluate_multi(length)
        angles = []
        if FIX_ANGLES:
            angles = [curve.evaluate_hodograph(i) for i in length]
            angles = [np.arctan2(x[1], x[0])[0] for x in angles]
        else:
            angles = [0 for _ in length]
...
def rtr_segment(wp1, wp2, s, model, r_step, path):
    # rotate (1)
...
def fix_angle_segment(FIX_ANGLES, part, path, wp2):
    if FIX_ANGLES:
...
class TrajectoryGenerator:
    ...
    def tuples_to_path(self, waypoints_tuples):
        return waypoints_to_path(
            waypoints_tuples, r=self.r, model=self.model, step=self.step
        )
    ...
    def compute_trajectory_objectives(self, trajectory):
        danger = np.max(
            [
                self.image[self.metric_to_px_coorditaes(x, y)] * 20 / (i + 20)
...
class VehicleModelNode(Node):
    def __init__(self):
        ...
        self.create_service(
            VehicleModelService, "nav/vehicle_model", self.vm_callback
        )
