...
class InflationNode(Node):
    def __init__(self):
        ...
        self.tfBuffer = tf2_ros.Buffer(
            cache_time=rclpy.time.Duration(seconds=3.0)
        )
        ...
        self.create_subscription(
            OccupancyGrid, "map", self.map_cb, qos_profile
        )
        self.map_publisher = self.create_publisher(
            OccupancyGrid, "nav/obstacles", 10
        )
        self.scan = None
        self.create_subscription(
    ...
    def scan_image(self):
        ...
        for i, r in enumerate(self.scan.ranges):
            if r > r_max:
                continue
            x = trans.transform.translation.y + r * np.sin(
                own_theta + i / 180.0 * np.pi
            )
            y = trans.transform.translation.x + r * np.cos(
                own_theta + i / 180.0 * np.pi
            )
    ...
    def metric_to_px_coorditaes(self, x, y):
        x = (
            x - self.raw_map.info.origin.position.x
        ) / self.raw_map.info.resolution
        y = (
            y - self.raw_map.info.origin.position.y
        ) / self.raw_map.info.resolution
        return int(x), int(y)

    def transform_to_px_coordinates(self, t):
        y = (
            t.transform.translation.x - self.raw_map.info.origin.position.x
        ) / self.raw_map.info.resolution
        x = (
            t.transform.translation.y - self.raw_map.info.origin.position.y
        ) / self.raw_map.info.resolution
        return int(x), int(y)

    def other_robot_image(self):
        ...
        for _, robot_frame in enumerate(self.robot_frames):
            try:
                ...
                other[x][y] = 100
                r = self.safety_dist * 2
                theta = yaw_from_orientation(trans.transform.rotation)
                x = trans.transform.translation.y + r * np.sin(theta)
                y = trans.transform.translation.x + r * np.cos(theta)
                x, y = self.metric_to_px_coorditaes(x, y)
                if x < 0 or x >= self.raw_map.info.width:
                    continue
                if y < 0 or y >= self.raw_map.info.height:
    ...
    def combine_sensor_sources(self):
        other = segmentation.expand_labels(
            self.other_robot_image(),
            distance=2.0 * self.safety_dist / self.raw_map.info.resolution,
        )
        image = segmentation.expand_labels(
            self.map_image,
            distance=self.safety_dist / self.raw_map.info.resolution,
        )
        scan_image = segmentation.expand_labels(
            self.scan_image(),
            distance=self.safety_dist / self.raw_map.info.resolution,
        )
    ...
    def timer_cb(self):
        if self.raw_map is not None:
            self.map_publisher.publish(
                self.convert_image_to_map_data(self.combine_sensor_sources())
            )
        else:
