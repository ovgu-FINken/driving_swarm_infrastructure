...
class PSOPlanner(DrivingSwarmNode):
    ...
    def pso(self, population=None, pop_size=20, c_social=1.4, c_cognitive=1.4, w=0.5, granularity_0=1):
        ...
        if population is None:
            ...
            for i, w in enumerate(np.linspace(0.1, 0.9, num=5)):
                population[i,0] = self.start_wp[0] * w + self.goal_wp[0] * (w-1.0)
                population[i,1] = self.start_wp[1] * w + self.goal_wp[1] * (w-1.0)
                population[i,2] = self.start_wp[2] * w + self.goal_wp[2] * (w-1.0)
        ...
        while time() < end_time: 
            n_gen += 1
            v_cognitive = c_cognitive * np.random.rand(*population.shape) * pbest - population
            v_cognitive = c_cognitive * np.random.rand(*population.shape) * pbest - population
            v_social = c_social * np.random.rand(*population.shape) * gbest - population
        ...
        wps = [(x,y,t,1.0) for x,y,t in zip(x[0::3], x[1::3], x[2::3])]
        self.get_logger().info(f'gen: {n_gen}')# < rclpy.time.Duration(seconds=0.5): 
    ...
    def compute_path(self):
        ...
        if self.current_trajectory is not None and ti != 0:
            current = [ (p.pose.position.x, p.pose.position.y, yaw_from_orientation(p.pose.orientation) ) for p in self.current_trajectory.poses[ti:] ]
        ...
        if current is not None and self.weighted_sum(current) < self.weighted_sum(best):
        ...
        self.send_path([Pose2D(x=x, y=y, theta=theta) for x,y,theta in best], ti=ti)
    ...
    def get_waypoints(self):
        ...
        if ti is None or ti == 0:
            ti = 0
            try:
                trans = self.tfBuffer.lookup_transform(self.reference_frame, self.own_frame, rclpy.time.Time().to_msg())
            ...
            start = (self.current_trajectory.poses[ti].pose.position.x,
                    ...
                    yaw_from_orientation(self.current_trajectory.poses[ti].pose.orientation))
            
        goal = (self.goal.pose.position.x, 
    ...
    def get_trajectory_index(self, trajectory, offset=0.0):
        NANO = 0.001 ** 3
        current_stamp = self.get_clock().now() + rclpy.time.Duration(seconds=offset)
        trajectory_start = rclpy.time.Time.from_msg(self.current_trajectory.header.stamp)
        rate = 1.0
