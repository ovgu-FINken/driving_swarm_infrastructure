...
class PSOPlanner(Node):
    def __init__(self):
        ...
        self.follow_action_client = ActionClient(
            self, FollowPath, "nav/follow_path"
        )
        ...
        for future in self.action_client_futures:
            if future.done():
                ...
                self.get_logger().info(
                    f"goal accepted: {self.current_gh.accepted}"
                )
                self.action_result_futures.append(
                    self.current_gh.get_result_async()
                )
    ...
    def mg_pso(self):
        ...
        for _ in range(n_gen):
            v_cognitive = (
                c_cognitive
                * np.random.rand(*population.shape)
                * np.nan_to_num(pbest - population)
            )
            # print("cognitive:")
            # print(v_cognitive)
            v_social = (
                c_social
                * np.random.rand(*population.shape)
                * np.nan_to_num(gbest - population)
            )
            # print("social:")
            # print(v_social)
            population_ = (
                w * v_t + v_cognitive + v_social + population
            ...
            p_g_inc = (
                ...
                - len(
                    [
                        1
                        for p in population
                        if granularity(p) > granularity(gbest)
                    ]
                )
                / pop_size
            )
            ...
            for i, p in enumerate(population):
                ...
                if granularity(gbest) < granularity(p):
                    ...
                    #    population[i,x] = np.random.rand()
                elif np.random.rand() < p_g_inc or granularity(
                    p
                ) < granularity(gbest):
    ...
    def compute_path(self):
        ...
        if self.weighted_sum(trivial) < self.weighted_sum(best):
            best = trivial
            self.get_logger().info(
                f"could not find better solution than trivial."
            )
        self.get_logger().info(
    ...
    def send_path(self, trajectory):
        ...
        self.action_client_futures.append(
