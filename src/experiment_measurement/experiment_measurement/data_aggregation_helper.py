...
def get_latest_in_interval(conf):
    ...
    res = conf.df.loc[
        conf.df.timestamp.le(conf.t) & conf.df.timestamp.gt(conf.t_prev)
    ]
    return res.iloc[-1]
...
def filter_tf_child_frame_id(conf):
    ...
    mask = conf.df["data"].map(
        lambda x: x.transforms[0].child_frame_id == conf.robot_name
    )

    conf_copy.df = conf.df[mask]
    return conf_copy


def quaternion_to_euler(data):
    """Calculate the euler representation of a quaternion."""
    quaternion = [data.w, data.x, data.y, data.z]
...
def amcl_calc_x_and_y(conf):
    ...
    conf_copy.df["x"] = conf_copy.df["data"].map(
        lambda x: x.pose.pose.position.x
    )
    conf_copy.df["y"] = conf_copy.df["data"].map(
        lambda x: x.pose.pose.position.y
    )
    return conf_copy
...
def tf_calc_x_and_y(conf):
    ...
    conf_copy.df = conf.df.copy()
    conf_copy.df["x"] = conf_copy.df["data"].map(
        lambda x: x.transforms[0].transform.translation.x
    )
    conf_copy.df["y"] = conf_copy.df["data"].map(
        lambda x: x.transforms[0].transform.translation.y
    )
    return conf_copy
