...
def start_rosbag(context, *args, **kwargs):
    rosbag_topics_file = LaunchConfiguration("rosbag_topics_file").perform(
        context
    )
    start_rosbag_cmd = []
    # TODO: -a einbinden
    ...
    if rosbag_topics_file != "NONE":
        qos_override_file = LaunchConfiguration(
            "qos_override_file", default=None
        ).perform(context)
        ...
        for topic in rosbag_config:
            if topic.startswith("/"):
                ...
                for robot in robots[: int(n_robots)]:
                    rosbag_topics.append(
                        "/" + str(robot["name"]) + "/" + topic.strip()
                    )
        ...
        if qos_override_file is not None:
            start_rosbag_cmd.append(
                ExecuteProcess(
                    cmd=["ros2", "bag", "record"]
                    + rosbag_topics
                    + ["--qos-profile-overrides-path", str(qos_override_file)],
                    output="screen",
                )
            )
        else:
...
def generate_launch_description():
    ...
    declare_rosbag_flag_cmd = DeclareLaunchArgument(
        "use_rosbag", default_value="False"
    )
